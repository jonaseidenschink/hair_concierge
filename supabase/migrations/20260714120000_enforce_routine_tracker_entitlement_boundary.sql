-- Tracker data is available only through the server's service-role boundary.
REVOKE ALL ON TABLE public.routine_logs FROM authenticated;
REVOKE ALL ON TABLE public.routine_log_products FROM authenticated;
REVOKE ALL ON TABLE public.tracker_nudge_dismissals FROM authenticated;

REVOKE ALL ON FUNCTION public.replace_routine_log(date, text, text, text, jsonb, uuid, bigint) FROM PUBLIC, anon, authenticated, service_role;
REVOKE ALL ON FUNCTION public.delete_routine_log(date, text, uuid, bigint) FROM PUBLIC, anon, authenticated, service_role;
...
CREATE FUNCTION public.replace_routine_log(
  p_user_id uuid, p_logged_on date, p_timezone text, p_day_type text,
  p_custom_activity_name text, p_products jsonb, p_client_session_id uuid, p_client_revision bigint
...
CREATE FUNCTION public.delete_routine_log(p_user_id uuid, p_logged_on date, p_timezone text, p_client_session_id uuid, p_client_revision bigint)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, pg_temp AS $$
...
REVOKE ALL ON FUNCTION public.delete_routine_log(uuid, date, text, uuid, bigint) FROM PUBLIC, anon, authenticated;
